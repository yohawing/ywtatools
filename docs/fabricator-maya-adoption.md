...
- Mayaが自動変換するtarget名と未作成namespaceをUndo開始前に拒否
- 参照nodeを含む一括renameはreference editを作らず編集前に全拒否
- 参照nodeでも実名が変わらない検索置換はUndo不要のread-only no-op
...
- Weight Clipboard JSONも非空identityと0～1の個別weightを読込前検証
- Weight Paste / Averageはlocked influenceを検出して曖昧な再配分を編集前拒否
- 選択jointをウェイト0で追加し、別skinClusterで使うjoint-global lockも保持
- 選択した未使用・unlocked influenceだけを安全に削除
...
- `ywtaPoseId` string 属性による改名に強い明示アドレス（周辺空白を正規化し、C0 / DELは拒否）
...
- locator/controlなどjoint以外の子DAG nodeを含む階層はmirror開始前に拒否
...
- 単純名はparent jointのnamespaceを継承し、親なしではcurrent namespaceに依存せずrootへ作成
- Mayaが自動変換する名前と未作成namespaceをUndo開始前に拒否
...
- 基本objectの単純名もcurrent namespaceに依存せずrootへ作成
...
- 分岐joint、参照joint、skinCluster / constraint / IK接続済みjoint、child TRSのlock / 入力接続は編集前拒否
...
- 分岐、同一位置の親子、参照、skinCluster / constraint / IK、animCurveを含む接続・lock channelを編集前拒否
...
- 親子sourceが混在してもDAG親から順に移動し、入力順に依存せず全sourceをtargetへ一致
...
- Shape Swap / Set Colorは参照controlへのreference editを作らず編集前拒否
...
- asset group単独選択時も子孫の表示meshを走査して必要なskin rootを追加
