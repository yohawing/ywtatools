...
stateからSnapshotへ応答する。これによりSnapshot応答と先行Acceptedのorderingを一つの受信順で定義する。Default
bootstrap consumerはreconcile完了後にだけ未開始Sessionを返し、DCC AdapterがLifecycleを開始するまでSessionを
Activeへ遷移させない。
