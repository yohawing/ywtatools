...
- UI ラベル、ボタン名、メニュー名、コード識別子は英語を許容する
- ツールチップ / アノテーションは日本語とし、目的・対象・重要な前提条件を記載する
- 説明文、ヘルプ、ドキュメント、コメント、docstring などの文章は日本語を基本とする
- API 名、製品名、ファイル形式名など固有の技術用語は原表記を維持してよい
- 既存資産の一括翻訳は行わず、新規・変更箇所から適用する
...
- TODO の自律実行や複数タスクの連続実行を依頼された場合、独立して検証できたタスクは個別確認を待たずにコミットし、次のタスクへ進む。失敗中・未完了・未検証のスライスはコミットしない
- commit の自律実行を許可されていても push は含まない。push はユーザーから明示的に依頼された場合だけ行う
