...
int ywta_hair_tube_generate(uint32_t vertex_count, const double* positions_xyz,
  ...
  try {
    ...
    const ywta::mesh_core::HairTubeGeneratedMeshResult generated =
        ywta::mesh_core::regenerate_hair_tube_fixed_density(cage.cage, target_segments);
    if (!generated.ok()) {
    ...
    output->source_station_count = cage.cage.source_station_count;
    output->max_fit_deviation = cage.cage.max_fit_deviation;
    output->max_source_distance = generated.mesh.max_source_distance;
    output->cubic_active = cage.cage.cubic_active ? 1 : 0;
    return 0;
