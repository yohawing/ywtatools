...
選択した出力フォルダはUXPの永続トークンとして保存し、Photoshopやプラグインを
再起動した後も自動復元します。フォルダの移動・削除や権限変更でトークンが無効に
なった場合は、安全に未選択へ戻るため、もう一度フォルダを選択してください。
...
PBR / Toonグループ認識、出力命名、出力フォルダの永続化、RGBA割り当て、
透明領域、Roughness反転を確認します。
