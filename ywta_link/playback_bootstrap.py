"""Playback SessionをRoomのephemeral slotからbootstrapする共通consumer。"""
...
class PlaybackBootstrapError(RuntimeError):
    """Playback slotまたはAuthority snapshotのbootstrap失敗。"""
...
def bootstrap_playback_session(
    ...
    raise PlaybackBootstrapError(f"Playback bootstrap failed after {config.max_attempts} attempts: {detail}") from last_error
...
def _bootstrap_attempt(
    ...
    config: PlaybackBootstrapConfig,
    peer_id: str,
) -> tuple[AuthorityHandoffTracker, PlaybackSessionConfig]:
...
def _receive_descriptor(
    ...
    _validate_playback_metadata(data["metadata"], config)
...
def _receive(client: object, deadline: float) -> Frame:
    ...
    if not math.isfinite(remaining) or remaining <= 0:
        raise PlaybackBootstrapError("Playback bootstrap timed out")
    try:
        frame = client.receive(timeout=remaining)
    except (FrameTimeout, TimeoutError) as error:
        raise PlaybackBootstrapError("Playback bootstrap timed out") from error
    except Exception as error:
        raise PlaybackBootstrapError(f"Playback bootstrap receive failed: {_error_text(error)}") from error
    if type(frame) is not Frame:
        raise PlaybackBootstrapError("bootstrap receive must return a Frame")
    return frame
